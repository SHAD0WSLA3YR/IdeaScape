 ...
 *
 * Production equivalent of the Vite dev middleware from src/server/aiProxy.ts.
 * Forwards client requests to NVIDIA NIM while keeping API keys server-side.
 *
 * Deployed at: /api/ai/generate
 */
...
function validateOrigin(req: IncomingMessage): { valid: boolean; allowedOrigin: string } {
  ...
  if (!allowedOrigin) {
    // No origin configured - allow all (development mode)
    return { valid: true, allowedOrigin: '*' };
  }

  const origin = req.headers.origin || req.headers.referer;
  if (!origin) {
    // No origin header - could be a direct API call, allow
...
function buildProviderConfig(
  ...
  const baseUrl = (process.env.NVIDIA_BASE_URL?.trim() || 'https://integrate.api.nvidia.com/v1') + '/chat/completions';
...
function sendJson(res: ServerResponse, status: number, data: AIProxyResponse, allowedOrigin: string) {
...
export default async function handler(req: IncomingMessage, res: ServerResponse) {
  ...
  if (!body.messages || !Array.isArray(body.messages) || body.messages.length === 0) {
    sendJson(res, 400, { error: 'messages array is required' }, allowedOrigin);
    return;
  }
  ...
  if (!process.env.NVIDIA_API_KEY?.trim()) {
    sendJson(res, 502, { error: 'NVIDIA API key not configured' }, allowedOrigin);
    return;
  }
