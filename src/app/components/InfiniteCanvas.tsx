...
import { applyContentEditableCommand } from '../utils/contentEditableFormat';
...
export function InfiniteCanvas() {
  ...
  const connectingFromNodeId = useCanvasStore((state) => state.connectingFromNodeId);
  const highlightedGroupId = useCanvasStore((state) => state.highlightedGroupId);
  ...
  const startSelectionBox = useCanvasStore((state) => state.startSelectionBox);
  const updateSelectionBox = useCanvasStore((state) => state.updateSelectionBox);
  const endSelectionBox = useCanvasStore((state) => state.endSelectionBox);
  const clearSelection = useCanvasStore((state) => state.clearSelection);
  const setHighlightedGroup = useCanvasStore((state) => state.setHighlightedGroup);
  ...
  const pointerMovedDuringGestureRef = useRef(false);
  const suppressDoubleClickUntilRef = useRef(0);
  const pendingTransformRef = useRef<Partial<typeof transform> | null>(null);
  const transformFrameRef = useRef<number | null>(null);

  const nodesById = useMemo(() => new Map(nodes.map(node => [node.id, node])), [nodes]);
  const uniqueNodes = useMemo(() => Array.from(nodesById.values()), [nodesById]);

  const scheduleTransformUpdate = useCallback((nextTransform: Partial<typeof transform>) => {
  ...
  const handleMouseDown = (e: React.MouseEvent) => {
    ...
    pointerMovedDuringGestureRef.current = false;
    setContextMenu(null);
    ...
    if (e.button === 1 || (e.button === 0 && e.altKey)) {
      ...
      // Don't clear connection selection on mousedown over connection UI — that would
      // unmount the delete button before its click event fires (bubble reaches canvas first).
      const isConnectionUi = target.closest('[data-connection-ui]');
      if (!isConnectionUi) {
  ...
  const handleMouseMove = (e: React.MouseEvent) => {
    if (pointerDownPosRef.current && !pointerMovedDuringGestureRef.current) {
      ...
      if (deltaX > 4 || deltaY > 4) {
        pointerMovedDuringGestureRef.current = true;
      }
    }

    if (isPanning && (e.buttons & 4) === 0) {
      // Middle button no longer pressed (can happen if mouseup occurred over iframe/layer).
      setIsPanning(false);
      return;
    }
  ...
  const handleMouseUp = (e: React.MouseEvent) => {
    if (pointerMovedDuringGestureRef.current) {
      // Prevent drag gestures from being interpreted as a double-click.
  ...
  useEffect(() => {
    const handlePaste = (event: ClipboardEvent) => {
      ...
      const nodeId = addNode(centerX - 100, centerY - 60);
      const urlPattern = /^(https?:\/\/|www\.)/i;
      ...
      if (urlPattern.test(pastedText)) {
        ...
        updateNode(nodeId, {
          content: {
            type: 'text',
            value: pastedText,
            title: pastedText.substring(0, 50) + (pastedText.length > 50 ? '...' : ''),
          },
        });
      }
    };
  ...
  }, [addNode, transform.scale, transform.x, transform.y, updateNode]);
  ...
  // Disable accidental text selection while drag-selecting or panning.
  useEffect(() => {
  ...
  // Global safety net so panning always ends even after iframe interactions.
  useEffect(() => {
    if (!isPanning) return;
        ...
        >
          ...
          {isConnecting && connectingFromNodeId && previewConnection && (
            <svg className="absolute inset-0 pointer-events-none" style={{ width: '100%', height: '100%', overflow: 'visible' }}>
              {(() => {
                const fromNode = nodesById.get(connectingFromNodeId);
          ...
          {uniqueNodes.map(node => {
            ...
            return (
              <div 
                ...
                data-node-type={node.content.type}
                style={{
