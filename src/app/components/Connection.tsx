...
export function Connection({
  ...
  const midpoint = {
    x: 0.125 * fromPoint.x + 0.375 * controlX1 + 0.375 * controlX2 + 0.125 * toPoint.x,
    y: 0.125 * fromPoint.y + 0.375 * controlY1 + 0.375 * controlY2 + 0.125 * toPoint.y,
  };
  ...
  return (
    ...
    >
      <path
        ...
        fill="none"
        className={`drop-shadow-sm ${isSelected ? 'text-blue-500' : 'text-black dark:text-white'}`}
      />
      ...
      {isSelected && (
        ...
        >
          <button
            type="button"
            className="inline-flex h-7 w-7 items-center justify-center rounded-md border border-input bg-background text-foreground shadow-sm transition-colors hover:bg-accent hover:text-accent-foreground"
